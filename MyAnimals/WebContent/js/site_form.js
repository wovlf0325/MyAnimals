var SITE_FORM = function(){
	...
	var itemRequiredCk = function($obj){
		...
		$obj.find('.form-group').each(function(){
			...
			if($(this).find('label i').hasClass('icon-required') === true){ // 필수입력 항
				switch(type[0]){
					case 'privacy':
					...
					case 'textarea':
						...
						break;
					case 'radio':
						if(!$(this).find('input:radio[name="' + this.id + '"]:checked').val()){
							required_ck = false;
							return false;
						}
						//기타 옵션 직접입력란 null체크
						$($(this).find('input:radio[name="' + this.id + '"]').each(function(){
							if($(this).is(":checked") === true){
								if($(this).data('type') === 'etc'){
									if($("input:text[name='" + type[0] + "_etc_" + type[1] + "']").val() === ''){
						...
						break;
					case 'checkbox':
						if($(this).find("input:checkbox[name='" + this.id + "[]']").is(":checked") === false){
							required_ck = false;
							return false;
						}

						//기타 옵션 직접입력란 null체크
						$(this).find("input:checkbox[name='" + this.id + "[]']").each(function(){
							if($(this).is(":checked") === true){
								if($(this).data('type') === 'etc'){
									if($("input:text[name='" + type[0] + "_etc_" + type[1] + "']").val() === ''){
										required_ck = 'etc_null';
				...
				switch(type[0]){
					case 'checkbox' :
						//기타 옵션 직접입력란 null체크
						$("input:checkbox[name='" + this.id + "[]']").each(function(){
							if($(this).is(":checked") === true){
								if($(this).data('type') === 'etc'){
									if($("input:text[name='" + type[0] + "_etc_" + type[1] + "']").val() === ''){
										required_ck = 'etc_null';
					...
					case 'radio' :
						//기타 옵션 직접입력란 null체크
						$("input:radio[name='" + this.id + "']").each(function(){
							if($(this).is(":checked") === true){
								if($(this).data('type') === 'etc'){
									if($("input:text[name='" + type[0] + "_etc_" + type[1] + "']").val() === ''){
										required_ck = 'etc_null';
			...
			$.ajax({
				...
				success : function(result){
					...
					if(result.msg == 'SUCCESS'){
						...
						if ( typeof MOBON != 'undefined' && result.mobon_registration_check) MOBON.CompleteSubmit();
