...
var SITE_BOOKING_PAYMENT = function(){
	...
	var initPayment = function(order_code){
		...
		var $auto_cacel_info = $('#auto_cancel_info');		// 자동취소 정보
	...
	var startPayment = function(){
		...
		if($form.find('._refund_confirm_agree').length > 0 && $form.find('._refund_confirm_agree').prop('checked') != true){		// 취소/환불 규정 동의 안되어있을경우
			alert(LOCALIZE.타이틀_취소환불규정에대한동의를해주세요());
			return false;
		}
	...
	var submit = function(t, type, i){
		switch(type){
			...
			case 'mobile': // Mobile 형태 form Q&A
				var data;
				var $_parent_review_form = t.parents('#mobile_review_form');
				if ( $_parent_review_form.length ) {
					data = t.parents('#mobile_review_form').serializeObject();
				} else {
